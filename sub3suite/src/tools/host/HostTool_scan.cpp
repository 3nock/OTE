    ...
    */
    ...
    for(int i = 0; i < status->activeScanThreads; i++)
    {
        ...
        case 0: // ACTIVE DNS
        {
            ...
            connect(scanner, &host::Scanner::scanResult, this, &HostTool::onScanResult_dns);
            connect(scanner, &host::Scanner::scanProgress, ui->progressBar, &QProgressBar::setValue);
            connect(scanner, &host::Scanner::scanLog, this, &HostTool::onScanLog);
            connect(cThread, &QThread::finished, this, &HostTool::onScanThreadEnded);
            connect(cThread, &QThread::finished, scanner, &host::Scanner::deleteLater);
            connect(cThread, &QThread::finished, cThread, &QThread::deleteLater);
            connect(this, &HostTool::stopScanThread, scanner, &host::Scanner::onStopScan);
            connect(this, &HostTool::pauseScanThread, scanner, &host::Scanner::onPauseScan);
            connect(this, &HostTool::resumeScanThread, scanner, &host::Scanner::onResumeScan, Qt::DirectConnection);
            cThread->start();
            break;
        }
        case 1: // ACTIVE PORT
        {
            ...
            scanner->startScan(cThread);
            scanner->moveToThread(cThread);
            connect(scanner, &port::Scanner::scanResult_host, this, &HostTool::onScanResult_port);
            connect(scanner, &port::Scanner::scanProgress, ui->progressBar, &QProgressBar::setValue);
            connect(scanner, &port::Scanner::scanLog, this, &HostTool::onScanLog);
            connect(cThread, &QThread::finished, this, &HostTool::onScanThreadEnded);
            connect(cThread, &QThread::finished, scanner, &port::Scanner::deleteLater);
            connect(cThread, &QThread::finished, cThread, &QThread::deleteLater);
            connect(this, &HostTool::stopScanThread, scanner, &port::Scanner::onStopScan, Qt::DirectConnection);
            connect(this, &HostTool::pauseScanThread, scanner, &port::Scanner::onPauseScan, Qt::DirectConnection);
            connect(this, &HostTool::resumeScanThread, scanner, &port::Scanner::onResumeScan, Qt::DirectConnection);
            cThread->start();
            break;
        }
        case 2: // ACTIVE PING
        {
            ...
            scanner->startScan(cThread);
            scanner->moveToThread(cThread);
            connect(scanner, &ping::Scanner::scanResult_host, this, &HostTool::onScanResult_ping);
            connect(scanner, &ping::Scanner::scanProgress, ui->progressBar, &QProgressBar::setValue);
            connect(scanner, &ping::Scanner::scanLog, this, &HostTool::onScanLog);
            connect(cThread, &QThread::finished, this, &HostTool::onScanThreadEnded);
            connect(cThread, &QThread::finished, scanner, &ping::Scanner::deleteLater);
            connect(cThread, &QThread::finished, cThread, &QThread::deleteLater);
            connect(this, &HostTool::stopScanThread, scanner, &ping::Scanner::onStopScan, Qt::DirectConnection);
            connect(this, &HostTool::pauseScanThread, scanner, &ping::Scanner::onPauseScan, Qt::DirectConnection);
            connect(this, &HostTool::resumeScanThread, scanner, &ping::Scanner::onResumeScan, Qt::DirectConnection);
            cThread->start();
        }
        }
    }
...
void HostTool::onReScan(QQueue<QString> targets){
    if(targets.isEmpty())
        return;
...
void HostTool::onScanThreadEnded(){
    ...
    if(status->activeScanThreads == 0)
    {
        ...
        if(!m_failedScans.isEmpty()){
            ...
            connect(failedScansDialog, &FailedScansDialog::reScan, this, &HostTool::onReScan);
            failedScansDialog->show();
        }
    }
}

void HostTool::scanSummary(){
    /* elapsed time */
