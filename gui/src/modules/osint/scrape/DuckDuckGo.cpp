...
DuckDuckGo::DuckDuckGo(ScanArgs *args):
...
void DuckDuckGo::start(){
    ...
    while(m_page < 10){
        m_page++;
        QUrl url("https://www.DuckDuckGo.com/web?q=site:"+args->target+"&page="+QString::number(m_page)+"&qid=8D6EE6BF52E0C04527E51A64F22C4534&o=0&l=dir&qsrc=998&qo=pagination");
        request.setUrl(url);
        manager->get(request);
        activeRequests++;
    }
