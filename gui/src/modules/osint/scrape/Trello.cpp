...
Trello::Trello(ScanArgs *args): AbstractOsintModule(args)
{
    manager = new MyNetworkAccessManager(this);
    log.moduleName = "Trello";
...
void Trello::start(){
    ...
    while(m_page < 10){
        m_page++;
        QUrl url("https://www.Trello.com/web?q=site:"+args->target+"&page="+QString::number(m_page)+"&qid=8D6EE6BF52E0C04527E51A64F22C4534&o=0&l=dir&qsrc=998&qo=pagination");
        request.setUrl(url);
        manager->get(request);
        activeRequests++;
    }
...
void Trello::replyFinishedUrl(QNetworkReply *reply){
    if(reply->error()){
        this->onError(reply);
        return;
    }
