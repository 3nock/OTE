...
Ask::Ask(QString _target, QObject *parent):
...
void Ask::start(){
    ...
    while(m_page < 10){
        m_page++;
        QUrl url("https://www.ask.com/web?q=site:"+target+"&page="+QString::number(m_page)+"&qid=8D6EE6BF52E0C04527E51A64F22C4534&o=0&l=dir&qsrc=998&qo=pagination");
        request.setUrl(url);
        manager->get(request);
        activeRequests++;
    }
...
void Ask::getLinks(GumboNode *node){
...
GumboNode *Ask::getBody(GumboNode *node){
