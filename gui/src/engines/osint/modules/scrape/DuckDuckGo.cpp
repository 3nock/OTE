...
DuckDuckGo::DuckDuckGo(QString _target, QObject *parent):
    AbstractOsintModule(parent)
{
    target = _target;
    manager = new QNetworkAccessManager(this);
    connect(manager, &QNetworkAccessManager::finished, this, &DuckDuckGo::replyFinished);
}
...
void DuckDuckGo::start(){
    ...
    while(m_page < 10){
        m_page++;
        QUrl url("https://www.DuckDuckGo.com/web?q=site:"+target+"&page="+QString::number(m_page)+"&qid=8D6EE6BF52E0C04527E51A64F22C4534&o=0&l=dir&qsrc=998&qo=pagination");
        request.setUrl(url);
        manager->get(request);
        activeRequests++;
    }
...
void DuckDuckGo::getLinks(GumboNode *node){
...
GumboNode *DuckDuckGo::getBody(GumboNode *node){
