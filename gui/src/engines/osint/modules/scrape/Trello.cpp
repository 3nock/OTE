...
Trello::Trello(QString _target, QObject *parent):
...
void Trello::start(){
    ...
    while(m_page < 10){
        m_page++;
        QUrl url("https://www.Trello.com/web?q=site:"+target+"&page="+QString::number(m_page)+"&qid=8D6EE6BF52E0C04527E51A64F22C4534&o=0&l=dir&qsrc=998&qo=pagination");
        request.setUrl(url);
        manager->get(request);
        activeRequests++;
    }
...
void Trello::getLinks(GumboNode *node){
...
GumboNode *Trello::getBody(GumboNode *node){
